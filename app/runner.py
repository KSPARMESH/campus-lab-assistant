...
The supervisor delegates tasks to the inventory and booking desk specialists.
"""
import time
...
def execute_run(claimed: Claimed, *, store: RunStore, db: LabDb, providers: dict,
    ...
    record_delay widens the gap between a tool's side effect and its record, for the crash drill.
    """
    ...
    if final_text is not None:                  # crashed after answer was produced, before complete
    ...
    while True:
        for step_seq, call in pending:
            ...
            if record_delay:
                time.sleep(record_delay)        # crash drill: side effect is done, record is not yet in store
