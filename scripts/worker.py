"""Run a worker process: polls the queue, claims runs with leases, executes multi-agent loops.

    python -m scripts.worker [--mock] [--slow 2] [--once]
"""
import argparse
import logging
import time
...
def main() -> None:
    ...
    p.add_argument("--mock", action="store_true", help="Use deterministic scripted providers")
    p.add_argument("--slow", type=float, default=0.0, help="Simulate latency per step")
    p.add_argument("--once", action="store_true", help="Process at most one run and exit")
    a = p.parse_args()
